...
class GameEngine {
    ...
    update() {
        ...
        for (let i = 0; i < updates; i++) {
            if (game.gameMode === 'campaign') {
                ...
                game.inputHandler.handlePlayerInput(game.player1, game.player1.controls);
                ...
                game.inputHandler.handlePlayerInput(game.player1, game.player1.controls);
                game.inputHandler.handlePlayerInput(game.player2, game.player2.controls);
            }
        }
        ...
        game.player1.x = Math.max(0, Math.min(game.width - game.player1.width, game.player1.x));
        game.player2.x = Math.max(0, Math.min(game.width - game.player2.width, game.player2.x));
        ...
        if (game.uiManager) {
            game.uiManager.updateHUD({
                ...
                stage: game.campaignManager ? game.campaignManager.currentStage + 1 : 1
            });
        }
    ...
    generateMovementEffects() {
        ...
        // Running dust
        if (Math.abs(game.player1.velocityX) > 3 && game.player1.isGrounded && !game.player1.isAttacking) {
            if (Math.random() < 0.3) {
        ...
        if (Math.abs(game.player2.velocityX) > 3 && game.player2.isGrounded && !game.player2.isAttacking) {
            if (Math.random() < 0.3) {
                game.particleSystem.createDustCloud(
                    game.player2.x + game.player2.width / 2,
