...
class CombatComponent {
    ...
    punch() {
        ...
        if (!player.isAttacking && player.attackCooldown <= 0 && player.hitStun <= 0 && !player.isKnockedDown) {
    ...
    crouch() {
        const player = this.player;
        if (player.isGrounded && !player.isAttacking && player.hitStun <= 0 && !player.isKnockedDown) {
    ...
    updateAttack(opponent) {
        ...
        if (player.attackFrame >= 5 && player.attackFrame <= 10) {
            ...
            const hitboxX = player.x + (player.facing === 1 ? player.width : -hitboxWidth);
