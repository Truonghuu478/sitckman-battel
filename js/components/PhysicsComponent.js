...
class PhysicsComponent {
    ...
    moveLeft() {
        const player = this.player;
        if (!player.isAttacking && player.hitStun <= 0 && !player.isKnockedDown && !player.isDashing) {
    ...
    jump() {
        const player = this.player;
        if (player.isGrounded && !player.isJumping && !player.isAttacking && player.hitStun <= 0 && !player.isKnockedDown) {
    ...
    dash() {
        const player = this.player;
        if (player.dashCooldown <= 0 && !player.isDashing && !player.isKnockedDown && 
            !player.isAttacking && player.hitStun <= 0 && player.isGrounded) {
