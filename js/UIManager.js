...
class UIManager {
  ...
  animateOverlayEnter(overlay, data) {
    ...
    const cards = overlay.querySelectorAll(
      ".glass-card, .stat-card, .upgrade-card, .stage-card",
    );
    if (cards.length > 0) {
      timeline.fromTo(
        cards,
    ...
    const buttons = overlay.querySelectorAll(
      ".btn-glass, .btn-base, .neon-button",
    );
    if (buttons.length > 0) {
      timeline.fromTo(
        buttons,
  ...
  updateHUD(playerData, opponentData, gameState) {
    ...
    this.updateHealthBar(
      "player2-health",
      opponentData.health / opponentData.maxHealth,
    );
    ...
    if (this.hudElements.timer && gameState.timer !== undefined) {
      ...
      if (gameState.timer <= 10) {
        this.hudElements.timer.classList.add(
          "text-neon-yellow",
          "animate-pulse-fast",
        );
      } else {
        this.hudElements.timer.classList.remove(
          "text-neon-yellow",
          "animate-pulse-fast",
        );
      }
    ...
    // Update combo counter (Phase 2 enhancement)
    if (playerData.combo && playerData.combo > 1) {
      this.showComboCounter(playerData.combo);
    }

    // Update stone counter (Phase 2 enhancement)
    if (playerData.stones !== undefined) {
      this.updateStoneCounter(playerData.stones);
    }
  }

  /**
   * Show combo counter in HUD (Phase 2)
   */
  showComboCounter(combo) {
    ...
    if (!comboEl) {
      ...
      comboEl.className =
        "fixed top-24 left-1/2 transform -translate-x-1/2 z-30";
      const header = document.getElementById("game-header");
      if (header && header.parentNode) {
        header.parentNode.insertBefore(comboEl, header.nextSibling);
      }
    }

    comboEl.innerHTML = `
            <div class="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border-2 border-yellow-500/60 rounded-xl px-6 py-2 backdrop-blur-md shadow-neon-yellow">
                <div class="flex items-center gap-2">
                    <span class="text-yellow-400 text-2xl font-black">${combo}</span>
                    <span class="text-yellow-300 text-lg font-bold uppercase">HITS!</span>
  ...
  /**
   * Update stone counter display (Phase 2)
   */
  updateStoneCounter(stoneCount) {
    let stoneEl = document.getElementById("stone-counter");

    if (!stoneEl) {
      stoneEl = document.createElement("div");
      stoneEl.id = "stone-counter";
      stoneEl.className = "fixed bottom-24 left-8 z-30";
      document.body.appendChild(stoneEl);
    }

    const stones = Array(3)
      .fill("🪨")
      .map((stone, i) =>
        i < stoneCount ? stone : '<span style="opacity: 0.2">🪨</span>',
      )
      .join(" ");

    stoneEl.innerHTML = `
            <div class="bg-slate-900/80 border border-slate-600/50 rounded-lg px-4 py-2 backdrop-blur-sm">
                <div class="text-sm text-slate-400 font-bold mb-1">STONES</div>
                <div class="text-2xl">${stones}</div>
            </div>
        `;
  }
  ...
  updateHealthBar(id, percentage) {
    const bar =
      this.hudElements[
        id === "player1-health" ? "player1Health" : "player2Health"
      ];
    const percentText =
      this.hudElements[
        id === "player1-health" ? "player1Percent" : "player2Percent"
      ];
  ...
  createStageCard(stage, index) {
    ...
    const difficultyColor =
      difficultyColors[stage.difficulty] || "text-cyan-400";
  ...
  showComboNotification(combo) {
    ...
    if (!comboEl) {
      ...
      comboEl.className =
        "fixed top-1/3 right-8 z-50 text-6xl font-black text-neon-yellow";
  ...
  /**
   * Setup upgrade shop button handlers (Phase 2)
   */
  setupUpgradeHandlers(player, coins) {
    const upgrades = [
      {
        id: "health",
        cost: 200,
        maxLevel: 5,
        apply: (lvl) => (player.maxHealth = 100 + lvl * 10),
      },
      {
        id: "attack",
        cost: 250,
        maxLevel: 5,
        apply: (lvl) => (player.punchDamage = 10 + lvl * 2),
      },
      {
        id: "speed",
        cost: 150,
        maxLevel: 5,
        apply: (lvl) => (player.speed = 5 + lvl * 0.5),
      },
      {
        id: "defense",
        cost: 300,
        maxLevel: 5,
        apply: (lvl) => (player.damageReduction = lvl * 0.1),
      },
    ];
      ...
      if (button) {
        button.addEventListener("click", () => {
          ...
          if (level >= upgrade.maxLevel) {
            this.showNotification("⚠️ Max level reached!", "warning");
            return;
          }
          ...
          // Purchase upgrade
          if (window.game) {
            window.game.purchaseUpgrade(upgrade.id, upgrade.cost);
            this.showNotification(
              `✅ ${upgrade.id.toUpperCase()} upgraded!`,
              "success",
            );
            this.animatePurchase(upgrade.id);
          }
        });
      }
    });
  ...
  /**
   * Animate successful purchase (Phase 2)
   */
  animatePurchase(upgradeId) {
    const card = document
      .querySelector(`#upgrade-${upgradeId}`)
      .closest(".upgrade-card");

    if (this.gsapLoaded && card) {
      // Flash effect
      gsap.to(card, {
  ...
  /**
   * Create particle effect for purchase (Phase 2)
   */
  createPurchaseParticles() {
    const colors = ["#6467f2", "#05d9e8", "#ff2a6d", "#facc15"];
    const container = document.querySelector(".game-container");
  ...
  /**
   * Show notification message (Phase 2)
   */
  showNotification(message, type = "info") {
    let notifEl = document.getElementById("notification-message");

    if (!notifEl) {
      notifEl = document.createElement("div");
      notifEl.id = "notification-message";
      notifEl.className =
    ...
    const colors = {
      ...
      info: "bg-cyan-500/20 border-cyan-500/60 text-cyan-400",
    };

    notifEl.innerHTML = `
            <div class="${colors[type]} border-2 rounded-xl px-6 py-3 backdrop-blur-md font-bold">
                ${message}
            </div>
        `;
  ...
  /**
   * Add screen shake effect for heavy hits (Phase 2)
   */
  screenShake(intensity = 10) {
    const canvas = document.getElementById("gameCanvas");
    if (!canvas || !this.gsapLoaded) return;
